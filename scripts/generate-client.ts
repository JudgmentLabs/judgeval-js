...
function getSchemaFromContent(content: any): string | null {
  ...
  if (content["application/json"]) {
    ...
    // If schema is empty object, it means no response body
    if (Object.keys(schema).length === 0) {
      return "EMPTY_SCHEMA";
    }
    // If no $ref but has type, we might need to handle inline schemas
    if (schema.type) {
      return null; // For now, we only handle $ref schemas
    }
  }

  // Try other common content types
...
function getResponseSchema(operation: any): string | null {
  ...
  // Try to find a response schema in success status codes first
  ...
  // If no success code found, try other 2xx status codes
  ...
  // Only as a last resort, try any response with content (but avoid error responses)
  for (const [statusCode, response] of Object.entries(responses)) {
    if (
...
function getTypeScriptType(
  ...
  // Handle schemas without explicit type - infer from properties
  if (!schema.type) {
    ...
    // If no type but has items, assume array
    if (schema.items) {
    ...
    // If no type and has additionalProperties, assume record
    ...
    // Default to any for schemas without clear type information
    return "any";
  }
...
function generateModelClass(className: string, schema: any): string {
  ...
  // Generate the TypeScript type for the schema
  const tsType = getTypeScriptType(schema, referencedTypes, visited);

  // Add imports for referenced types (excluding self-reference)
