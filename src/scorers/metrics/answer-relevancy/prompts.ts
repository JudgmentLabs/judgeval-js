...
export const ARVerdictSchema = z.object({
  verdict: z.string(),
  reason: z.string()
});
...
export const VerdictsSchema = z.object({
  verdicts: z.array(ARVerdictSchema)
});
 ...
 */
export class AnswerRelevancyTemplate {
  /**
...
JSON:`;
  ...
  static generateVerdicts(input: string, statements: string[]): string {
    return `You will be provided with a list of statements from a response; your task is to determine whether each statement is relevant with respect to a provided input.
...
JSON:`;
  ...
  static generateReason(irrelevantStatements: [string, string][], input: string, score: string): string {
    const irrelevantStatementsText = irrelevantStatements.map(([statement, reason]) => 
      `statement: ${statement}\nreason: ${reason}\n------`
    ).join('\n');
